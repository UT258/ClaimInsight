  ...
  return (
    <div>
      ...
      <div style={styles.chartRow}>
        <DataCard title="Reserve trend" subtitle="Monthly total set aside">
          {reserveTrend.length === 0 ? (
            <EmptyState title="No trend data" description="Monthly totals will appear as reserves are booked." />
          ) : (
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={reserveTrend} margin={{ top: 10, right: 16, bottom: 0, left: -10 }}>
                <defs>
                ...
                <XAxis dataKey="month" tick={{ fontSize: 10, fill: 'var(--ci-text-secondary)' }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 10, fill: 'var(--ci-text-muted)' }} tickFormatter={v => fmtUSDShort(Number(v))} axisLine={false} tickLine={false} />
                <RTooltip
                  cursor={{ stroke: 'var(--ci-border-strong)' }}
                  formatter={(v) => fmtUSD(Number(v))}
                  contentStyle={{ background: 'var(--ci-bg-surface)', border: '1px solid var(--ci-border)', borderRadius: 8, fontSize: 12 }}
                />
                <Area type="monotone" dataKey="total" stroke={CHART.teal} strokeWidth={2} fill="url(#resGrad)" dot={{ r: 3, fill: CHART.teal }} />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </DataCard>

        <DataCard title="Top 10 reserves" subtitle="Largest amounts booked">
          {topClaims.length === 0 ? (
            <EmptyState title="No reserves" description="Reserved claims will appear here." />
          ) : (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={topClaims} layout="vertical" margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--ci-border)" horizontal={false} />
                <XAxis type="number" tick={{ fontSize: 10, fill: 'var(--ci-text-muted)' }} tickFormatter={v => fmtUSDShort(Number(v))} axisLine={false} tickLine={false} />
                <YAxis dataKey="name" type="category" width={90} tick={{ fontSize: 10, fill: 'var(--ci-text-secondary)' }} axisLine={false} tickLine={false} />
                <RTooltip
                  cursor={{ fill: 'var(--ci-bg-surface-2)' }}
                  formatter={(v) => fmtUSD(Number(v))}
                  contentStyle={{ background: 'var(--ci-bg-surface)', border: '1px solid var(--ci-border)', borderRadius: 8, fontSize: 12 }}
                />
                <Bar dataKey="amount" radius={[0, 5, 5, 0]} barSize={14}>
                  {topClaims.map((d) => {
                    const color = d.amount >= 100_000 ? CHART.red : d.amount >= 10_000 ? CHART.amber : CHART.teal;
