...
import { claimsApi, ClaimKpi, METRIC_NAMES } from '../../api/claimsApi';
import {
...
type Action =
  ...
  | { type: 'SET_CALC_CLAIM'; payload: string }
  | { type: 'CALC_START' }
  | { type: 'CALC_SUCCESS'; payload: ClaimKpi[] };

const initial: State = {
  items: [], loading: false, error: null,
  modalOpen: false, submitting: false,
  filterMetric: null, calcClaimId: '', calcLoading: false,
};
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'SET_CALC_CLAIM':     return { ...state, calcClaimId: action.payload };
    case 'CALC_START':         return { ...state, calcLoading: true };
    case 'CALC_SUCCESS':       return { ...state, calcLoading: false, items: [...state.items, ...action.payload] };
...
export default function ClaimsPage() {
  ...
  const handleCalculate = async () => {
    if (!state.calcClaimId.trim()) return;
    dispatch({ type: 'CALC_START' });
    try {
      const records = await claimsApi.calculateForClaim(state.calcClaimId);
      dispatch({ type: 'CALC_SUCCESS', payload: records });
    } catch {
      dispatch({ type: 'CALC_SUCCESS', payload: [] });
    }
  };
  ...
  return (
    <div>
      ...
      {/* Auto-calculate strip */}
      <Card style={{ marginBottom: 16, background: '#f0f9ff', border: '1px solid #bae0ff' }}>
        <Space wrap>
          <Text strong>Auto-calculate all KPIs for a claim:</Text>
          <Input
            placeholder="Claim ID (e.g. CLM-2026-AUTO-001)"
            style={{ width: 260 }}
            value={state.calcClaimId}
            onChange={e => dispatch({ type: 'SET_CALC_CLAIM', payload: e.target.value })}
          />
          <Button
            type="primary"
            icon={<ThunderboltOutlined />}
            loading={state.calcLoading}
